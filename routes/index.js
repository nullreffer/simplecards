...
router.get('/', async (req, res, next) => {
  ...
  if (req.query.game) games = games.filter(g => g.name == req.query.game);
  res.render('index', { joingameid: req.query.joingameid, randomname: randomname, isAdmin: req.query.godmode, games: games });
});
...
router.route('/newgame').post(async (req, res, next) => {
  try {
    ...
    res.redirect('/?joingameid=' + created.name + '&game=' + created.name);
...
router.route('/joingame').post(async (req, res, next) => {
  ...
  } catch (err) { res.redirect("/?game=" + joingameid + "&err=" + err.response.data.message); }
});
